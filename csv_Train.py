...
how_many_1_Test = 0
how_many_0_Test = 0
# Zliczanie liczby plikow w folderze Train_xml:
path, dirs, files = next(os.walk("./Train_xml"))
...
for filename in glob2.glob(os.path.join(path, '*.xml')):
   with open(os.path.join(os.getcwd(), filename), 'r') as f:
    ...
    for i in root2:
        ...
        rows.append([amount, Width, Height, RoiX1, RoiY1, RoiX2, RoiY2, ClassId, 'Train/' + Path])
    df = pd.DataFrame(rows, columns=cols)

    # Tworzenie pliku .csv i zapis do niego:
    df.to_csv('Train.csv')
print("Liczba plikow w Trainxml:", file_count)
print("Liczba znakow crosswalks Train:", how_many_1)
print("Liczba znakow innych niz crosswalks Train:", how_many_0)
print(Width)